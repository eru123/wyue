...
class Cmd
{
    public static function noshell_exec(string $cmd): false|string
    {
    ...
    public static function escape_win32_argv(string $value): string
    {
      ...
      | \\\++ (?=("|$)) # backslashes followed by a quote or at the end
        ...
        if (null === $escaped) {
            throw PREG_BAD_UTF8_ERROR === preg_last_error()
    ...
    public static function cmdp(array|string $cmd): string
    {
        return is_array($cmd) ? implode(' ', array_map(PHP_OS_FAMILY === 'Windows' ? [static::class, 'escape_win32_argv'] : 'trim', $cmd)) : $cmd;
    }

    public static function cmd(array|string $cmd, $parallel = false): null|false|string
    {
        $cmd = static::cmdp($cmd);
    ...
    public static function xshell(array|string $cmd): null|false|string
    {
