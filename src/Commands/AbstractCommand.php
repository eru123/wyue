...
abstract class AbstractCommand
{
     ...
     * @return mixed
     */
    public function args(?string $key = null, $default = null)
    {
        $args = CLI::args();
        array_shift($args);
     ...
     * @return mixed
     */
    public function arg(?string $key = null, $default = null)
    {
        $args = CLI::args();
        array_shift($args);
     ...
     * @return mixed
     */
    public function opt(?string $key = null, $default = null)
    {
    ...
    public function translateKeyFlags(array $haystack): array
    {
        ...
        foreach ($haystack as $k => $v) {
            $kas = explode('|', $k);
            $kat = array_map(fn ($i) => 1 == strlen($i) ? "-{$i}" : "--{$i}", $kas);
            $kak = implode('|', $kat);
            $res[$kak] = $v;
        }
