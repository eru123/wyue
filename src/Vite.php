...
class Vite
{
     ...
     */
    public const TEMPLATE_REACT = 'react';

    /**
     * @var string Use Vite Template for production
     */
    public const TEMPLATE_VITE = 'vite';

    /**
     * @var string Use Vite Template for development
     */
    public const TEMPLATE_DEV = 'dev';

    protected static $str_tpl_name;
    protected static $str_template;
    protected static $str_manifest;
    protected static $arr_head = [];
    protected static $arr_body = [];
    protected static $arr_data = [];
    protected static $dir_src;
    protected static $dir_pub;
    protected static $dir_dis;
    ...
    public static function render(array $data = [], bool $minify = true): string
    {
        ...
        if (!isset($tpl_data['entry'])) {
            $tpl_data['entry'] = ['src/main.js', 'src\\main.js'];
        }
        ...
        if (!$dev) {
            ...
            if (is_array($entry)) {
                foreach ($entry as $tmp_entry) {
                    if (isset($manifest[$tmp_entry]) && isset($manifest[$tmp_entry]['isEntry']) && $manifest[$tmp_entry]['isEntry']) {
                        $entry = $tmp_entry;
                        break;
                    }
                }
            ...
            } else if (!isset($manifest[$entry]) || !isset($manifest[$entry]['isEntry']) || !$manifest[$entry]['isEntry']) {
                throw new \Exception('Invalid manifest file or entry file path');
            }

            $css = isset($manifest[$entry]['css']) ? $manifest[$entry]['css'] : [];
            $js = isset($manifest[$entry]['imports']) ? $manifest[$entry]['imports'] : [];
            $tpl_data['entry'] = $manifest[$entry]['file'];

            if (empty($css)) {
                if (is_array($tpl_data['entry'])) {
                    foreach ($tpl_data['entry'] as $entry_file) {
                        if (isset($manifest[$entry_file]) && !isset($manifest[$entry_file]['isEntry']) && str_ends_with($manifest[$entry_file]['file'], '.css')) {
                            $css = [$manifest[$entry_file]['file']];
                            break;
                        }
                    }
                }
            ...
            foreach ($js as $js_file) {
                $jsf = $manifest[$js_file]['file'] ?? null;
                if ($jsf) {
