...
class PHPMailer
{
    ...
    public function preSend()
    {
        if (
            'smtp' === $this->Mailer
            || ('mail' === $this->Mailer && (\PHP_VERSION_ID >= 80000 || 0 === stripos(PHP_OS, 'WIN')))
        ) {
    ...
    public function postSend()
    {
        try {
            ...
            if ('smtp' === $this->Mailer && true == $this->SMTPKeepAlive && $this->smtp->connected()) {
                $this->smtp->reset();
            }
            if ($this->exceptions) {
                throw $exc;
    ...
    public function setSMTPXclientAttribute($name, $value)
    {
        ...
        if (isset($this->SMTPXClient[$name]) && null === $value) {
    ...
    public static function isValidHost($host)
    {
        ...
        return false !== filter_var('http://'.$host, FILTER_VALIDATE_URL);
    }
    ...
    public function addCustomHeader($name, $value = null)
    {
        ...
        if (empty($name) || false !== strpbrk($name.$value, "\r\n")) {
            if ($this->exceptions) {
                throw new \Exception($this->lang('invalid_header'));
            }
    ...
    protected function sendmailSend($header, $body)
    {
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                $this->edebug('Result: '.(0 === $result ? 'true' : 'false'));
                if (0 !== $result) {
                    throw new \Exception($this->lang('execute').$this->Sendmail, self::STOP_CRITICAL);
                }
            }
